  ...
  async function onSubmit(values: z.infer<typeof loginSchema>) {
    ...
    try {
      ...
      if (user) {
        ...
        const rolePath = user.role === 'Inventory Manager' ? 'inventory' : user.role.toLowerCase();
        router.push(`/${rolePath}/dashboard`);
      }
    } catch (error) {
      const err = error as Error;
      if (err.message.includes("Account status: Pending")) {
        toast.error("Your account is pending admin approval.");
      } else if (err.message.includes("Account status: Rejected")) {
